...
class UsageTracker:
    ...
    def __init__(self, user_id, user_name):
        ...
        :param supabase_client: Instance of the Supabase client
        ...
        user_data = self.supabase.table("users").select("*").eq("user_id", self.user_id).execute()
        if not user_data.data:
            # Insert new user
            self.supabase.table("users").insert({"user_id": self.user_id, "user_name": self.user_name}).execute()
    ...
    def initialize_or_fetch_current_cost(self):
        current_cost_data = self.supabase.table("current_costs").select("*").eq("user_id", self.user_id).execute()
        if not current_cost_data.data:
            # Initialize cost data for a new user
            current_cost = {"day": 0.0, "month": 0.0, "all_time": 0.0, "last_update": datetime.now().isoformat()}
    ...
    def fetch_usage_history(self):
        usage_history_data = self.supabase.table("usage_history").select("*").eq("user_id", self.user_id).execute()
    ...
    def add_chat_tokens(self, tokens, tokens_price=0.002):
        ...
        usage_history = self.supabase.table("usage_history").select("*").eq("user_id", self.user_id).execute().data
        if usage_history:
            ...
            self.supabase.table("usage_history").update({"chat_tokens": chat_tokens}).eq("user_id",
                                                                                         self.user_id).execute()
        else:
            # Initialize chat_tokens history
            self.supabase.table("usage_history").insert({
                "user_id": self.user_id,
                "chat_tokens": {today: tokens}
            }).execute()
    ...
    def get_current_token_usage(self):
        ...
        if usage_history:
            ...
            usage_month = sum(tokens for day, tokens in chat_tokens.items() if day.startswith(month_str))
        else:
    ...
    def add_current_costs(self, request_cost):
        ...
        current_cost_data = self.supabase.table("current_costs").select("*").eq("user_id", self.user_id).execute().data
        if current_cost_data:
            ...
            self.supabase.table("current_costs").update(current_cost).eq("user_id", self.user_id).execute()
        else:
    ...
    def add_image_request(self, image_size, image_prices="0.016,0.018,0.02"):
        ...
        if usage_history:
            ...
            # Update the usage_history in the database
            self.supabase.table("usage_history").update({"number_images": number_images}).eq("user_id",
                                                                                             self.user_id).execute()
        else:
            # Initialize number_images history
            self.supabase.table("usage_history").insert({
                "user_id": self.user_id,
                "number_images": {today: [0 if i != requested_size else 1 for i in range(3)]}
            }).execute()
    ...
    def get_current_image_count(self):
        ...
        if usage_history:
            ...
            usage_month = sum(sum(images) for day, images in number_images.items() if day.startswith(month_str))
        else:
            usage_day, usage_month = 0, 0
    ...
    def add_vision_tokens(self, tokens, vision_token_price=0.01):
        ...
        if usage_history:
            ...
            # Update the usage_history in the database
            self.supabase.table("usage_history").update({"vision_tokens": vision_tokens}).eq("user_id",
                                                                                             self.user_id).execute()
        else:
            # Initialize vision tokens history
            self.supabase.table("usage_history").insert({
                "user_id": self.user_id,
                "vision_tokens": {today: tokens}
            }).execute()
    ...
    def get_current_vision_tokens(self):
        ...
        if usage_history:
            ...
            tokens_month = sum(tokens for day, tokens in vision_tokens.items() if day.startswith(month_str))
        else:
    ...
    def add_tts_request(self, text_length, tts_model, tts_prices):
        ...
        if usage_history:
            ...
            tts_characters[tts_model][today] = tts_characters[tts_model].get(today, 0) + text_length

            # Update the usage_history in the database
            self.supabase.table("usage_history").update({"tts_characters": tts_characters}).eq("user_id",
                                                                                               self.user_id).execute()
        else:
    ...
    def get_current_tts_usage(self):
        ...
        if usage_history:
            ...
            for tts_model, dates in tts_characters.items():
                ...
                characters_month += sum(characters for day, characters in dates.items() if day.startswith(month_str))
        else:
    ...
    def add_transcription_seconds(self, seconds, minute_price=0.006):
        ...
        if usage_history:
            ...
            # Update the usage_history in the database
            self.supabase.table("usage_history").update({"transcription_seconds": transcription_seconds}).eq("user_id",
                                                                                                             self.user_id).execute()
        else:
            # Initialize transcription_seconds history
            self.supabase.table("usage_history").insert({
                "user_id": self.user_id,
                "transcription_seconds": {today: seconds}
            }).execute()
    ...
    def get_current_transcription_duration(self):
        ...
        if usage_history:
            ...
            seconds_month = sum(seconds for day, seconds in transcription_seconds.items() if day.startswith(month_str))

        minutes_day, remaining_seconds_day = divmod(seconds_day, 60)
        minutes_month, remaining_seconds_month = divmod(seconds_month, 60)
        return int(minutes_day), round(remaining_seconds_day, 2), int(minutes_month), round(remaining_seconds_month, 2)
    ...
    def get_current_cost(self):
        ...
        if current_cost_data:
            ...
            cost_month = current_cost["month"] if today[:7] == last_update.isoformat()[:7] else 0.0
    ...
    def initialize_all_time_cost(self, tokens_price=0.002, image_prices="0.016,0.018,0.02", minute_price=0.006,
        ...
        total_images = [sum(values) for values in zip(*usage_history['number_images'].values())]
        ...
        total_transcription_seconds = sum(usage_history['transcription_seconds'].values())
        ...
        tts_cost = sum([sum(tts_model.values()) * price / 1000 for tts_model, price in
                        zip(usage_history['tts_characters'].values(), tts_prices_list)])

        # Sum all costs
        all_time_cost = token_cost + image_cost + transcription_cost + vision_cost + tts_cost

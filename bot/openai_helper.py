...
from config import BotConfig, GPT_3_MODELS, GPT_4_VISION_MODELS, GPT_4_128K_MODELS
...
class OpenAIHelper:
    ...
    def __init__(self, config: BotConfig, plugin_manager: PluginManager):
        """
        ...
        self.config = config
        http_client = (
            httpx.AsyncClient(proxies=self.config.proxy)
            if self.config.proxy is not None
            else None
        )
        self.client = openai.AsyncOpenAI(
            api_key=config.openai_api_key, http_client=http_client
        )
    ...
    async def __handle_function_call(
            ...
            async for item in response:
                if len(item.choices) > 0:
                    ...
                    if first_choice.delta and first_choice.delta.function_call:
                        ...
                        logging.info(
                            f" the response is :{response}, the plug has been used {plugins_used}"
                        )
        ...
        function_response = await self.plugin_manager.call_function(
            function_name, self, **json.loads(arguments)
        )
    ...
    async def generate_speech(self, prompt: str) -> tuple[any, int]:
        """
    ...
    async def __common_get_chat_response_vision(
        ...
        Request a response from the GPT model.
        :param chat_id: The chat ID
        :param content: The query to send to the model
        :return: The answer from the model and the number of tokens used
        """
        query = ""
        bot_language = self.config.bot_language
        try:
            ...
            common_args = {
                # several choices not implemented yet
    ...
    async def interpret_image(self, chat_id, file_obj, prompt=None):
        """
        Interprets a given PNG image file using the Vision model.
        """
        image = encode_image(file_obj)
    ...
    async def interpret_image_stream(self, chat_id, file_obj, prompt=None):
        """
        Interprets a given PNG image file using the Vision model.
        """
        image = encode_image(file_obj)
    ...
    def __count_tokens_vision(self, image_bytes: bytes) -> int:
        ...
        if w > h:
            w, h = h, w
        # this computation follows https://platform.openai.com/docs/guides/vision and
        # https://openai.com/pricing#gpt-4-turbo
        base_tokens = 85
